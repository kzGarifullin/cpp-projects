...
{
	m_image.create(m_calculator.width(), m_calculator.height());//имаг в текстуру потом в спрайт и рисуем

	if (!m_font.loadFromFile("consolas.ttf"))//шрифт
	{
...
void Visualizer::run()
{
	...
	{
		review();//елси пользователь нажал на кнопки
		update();//пересчет фрактала еслибыл ревью
		render();//отрисовка 
	}
}
...
void Visualizer::review()
{
	...
	while (m_window.pollEvent(event))
	{
		...
		case sf::Event::KeyPressed:
			m_need_redraw = true;
			handle_key_pressed(event.key.code);//обрабат навжатие кнопки
			break;
		default:
			break;
		}
...
{
	if (m_need_redraw) //если флаг выставлен вызываем и передаем новые данные
	{
...
void Visualizer::render()
{
	...
	const auto& field = m_calculator.field();///запрашиваем массив двумерный
	...
	if (m_need_redraw)
	{
		for (auto y = 0U; y < m_calculator.height(); ++y)
		{
			for (auto x = 0U; x < m_calculator.width(); ++x)
			{
				...
				{
					m_image.setPixel(x, y, sf::Color::Black);//проставляем цвета
				}
				else
				{
